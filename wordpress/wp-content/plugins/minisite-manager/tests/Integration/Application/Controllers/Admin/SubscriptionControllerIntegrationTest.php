...
final class SubscriptionControllerIntegrationTest extends TestCase
{
    ...
    private function createMockOrder(): object
    {
        return new class {
            ...
            public function get_meta($key) {
                $meta = [
                    '_minisite_id' => 'test-minisite-' . uniqid(),
                    '_slug' => 'test-business-' . uniqid() . '/test-location-' . uniqid(),
                    '_reservation_id' => 'reservation-' . uniqid()
                ];
    ...
    {
        $this->markTestSkipped('wp_die function causes issues in integration tests - needs proper error handling');
    }
    ...
    {
        $this->markTestSkipped('Requires complex WooCommerce and NewMinisiteController mocking - better suited for end-to-end tests');
    }
    ...
    public function testGetPendingMinisiteOrders(): void
    {
        ...
        $this->assertStringStartsWith('test-minisite-', $result[0]['minisite_id']);
        $this->assertStringContainsString('test-business-', $result[0]['slug']);
        $this->assertStringStartsWith('reservation-', $result[0]['reservation_id']);
        $this->assertEquals('Credit Card', $result[0]['payment_method']);
    }

    public function testGetCompletedMinisiteOrders(): void
    {
        $this->markTestSkipped('Requires wp_minisite_payments table which is not set up in test database');
    }
