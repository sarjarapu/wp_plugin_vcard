...
final class VersionManagementWorkflowIntegrationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Set up $wpdb object (minimal setup - not used in skipped tests)
        ...
        // TODO: MinisiteRepository setup removed - will be fixed separately
        // Create service (using mock MinisiteRepository for now)
        $wordPressManager = new WordPressVersionManager(new WordPressTerminationHandler());
        // Note: VersionService requires MinisiteRepository - these tests are commented out until minisite issues are fixed
     ...
     * TODO: Re-enable when MinisiteRepository issues are fixed
     */
    public function test_version_lifecycle_from_creation_to_rollback(): void
    {
        $this->markTestSkipped('Skipped until MinisiteRepository issues are fixed');

        /* Original implementation - restore when MinisiteRepository is fixed
        ...
        $createCommand = new CreateDraftCommand(
            ...
            siteJson: json_encode(array('sections' => array('hero' => array('title' => 'Welcome'))))
        );
        ...
        $createCommand2 = new CreateDraftCommand(
            ...
            siteJson: json_encode(array('sections' => array('hero' => array('title' => 'Updated Welcome'))))
        );
    ...
    public function test_version_listing_with_multiple_versions(): void
    {
        ...
        for ($i = 1; $i <= 5; $i++) {
            $createCommand = new CreateDraftCommand(
                ...
                siteJson: json_encode(array('version' => $i))
            );
        ...
        $this->assertEquals('published', $versions[3]->status); // Version 2
    ...
    public function test_rollback_creates_new_draft_from_source(): void
    {
        ...
        $createCommand = new CreateDraftCommand(
            ...
            label: 'Version 1',
            comment: 'First version',
            siteJson: json_encode(array('title' => 'Version 1 Title'))
        );
        ...
        $createCommand2 = new CreateDraftCommand(
            ...
            siteJson: json_encode(array('title' => 'Version 2 Title'))
        );
